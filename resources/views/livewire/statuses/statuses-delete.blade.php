<div class="d-inline">
    <a class="btn btn-sm btn-danger rounded-circle" data-bs-toggle="modal" data-bs-target="#deleteModalStatus{{$status['id']}}" title="{{ __('Delete') }}">
        <i class="fa fa-trash"></i>
    </a>

    <div wire:ignore.self class="modal fade  text-ddd" id="deleteModalStatus{{$status['id']}}" tabindex="-1" role="dialog" aria-labelledby="deleteModalStatus{{$status['id']}}Label" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content bg-sub">
                <div class="modal-header bg-sub border-secondary">
                    <h5 class="modal-title" id="deleteModal{{$status['id']}}Label">{{ __('Delete Confirm') }}</h5>
