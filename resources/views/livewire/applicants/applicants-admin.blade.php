                ...
                <div class="col-md-4">
                    <div class="p-2 position-relative  border-bottom border-secondary">
                        ...
                        <div class="progress bg-main-1">
                            @php
                                $applicants0 = App\Models\Cart::where('status', $key)->count();
                                if($key == 0){
                                    $bg = "info";
                                 } elseif($key == 1){
                                    $bg = "info";
                    ...
                    <form>
                        <div class="row">
                            <div class="col-md-4 my-auto">
                                <h5 class=" px-2 mb-0">
                                    <img src="{{ asset('assets/images/applicants.png') }}" width="35px" height="35px" class="contain ms-2" alt="">{{ __('All Applicants') }} : ({{ $applicants->count() }})
                                </h5>
                            </div>
                        </div>
                        <div class="row justify-content-center">
                            <div class="col-md-8 my-auto text-center">
                                    <div class="dropdown w-100 text-start my-3">
                                        ...
                                        <div class="dropdown-menu mx-auto text-center w-100 rounded-2 mt-1 shadow-sub bg-sub border-0" aria-labelledby="dropdownMenuLink">
                                            <div class="row drop-applicants drop-applicant-js w-100 mx-auto p-2">
                                                <div class="col-4 border-start border-2 border-secondary">
                                                    <div class="px-3 text-end">
                                                        <h5 class="text-ddd">{{ __('Users') }} :</h5>
                                                        @foreach($users as $user)
                                                            <div class="form-check pb-1 my-2 border-bottom border-secondary border-2">
                                                                <input class="form-check-input float-start" type="checkbox" name="user_id[]" value="{{ $user->id }}" {{in_array($user->id, (array)request('user_id')) ? 'checked' : ''}}>
                                                                <label class="form-check-label text-ddd h6 mb-0 me-0 me-md-4">{{ $user->name }}</label>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="px-3 text-end">
                                                        ...
                                                        @foreach($services as $service)
                                                            <div class="form-check pb-1 my-2 border-bottom  border-secondary border-2">
                                                                ...
                                                                <label class="form-check-label text-ddd h6  me-0 me-md-4">{{ $service->name_lang }}</label>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="px-3 text-end">
                                                        ...
                                                        @foreach($packages as $package)
                                                            <div class="form-check pb-1 my-2 border-bottom border-secondary border-2">
                                                                ...
                                                                <label class="form-check-label text-ddd h6  me-0 me-md-4">{{ $package->name_lang }}</label>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                </div>
                            ...
                            @if($applicants->count() > 0)
                                <table class="table table-striped border-secondary table-hover text-center">
                                    ...
                                    <tr>
                                        ...
                                        <th class="text-center text-ddd" scope="col">{{ __('Edit Status') }}</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @php
                                    ...
                                    @foreach($applicants as $key => $applicant)
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle text-ddd text-center">
                                                <livewire:applicants.applicants-show :applicant_id="$applicant->id" :key="'applicants-show-applicants-admin-'.$applicant->id"></livewire:applicants.applicants-show>
                                            </td>
                                            <td class="align-middle text-ddd text-center">
                                                <livewire:applicants.data-applicants :applicant_id="$applicant->id" :key="'data-applicants-applicants-admin-'.$applicant->id"></livewire:applicants.data-applicants>
                                            </td>
                                            <td class="align-middle text-center">
                                                <livewire:applicants.applicants-edit :applicant_id="$applicant->id" :key="'applicants-edit-applicants-admin-'.$applicant->id"></livewire:applicants.applicants-edit>
                                            </td>
                                        </tr>
                                    @endforeach
                                    <tr class="bg-sub">
                                        <td colspan="7" class=" border-0 bg-sub"></td>

                                        <td colspan="4" class="bg-main-1 border-0">
                                        ...
                                        <td colspan="4" class="bg-main-1 border-0">
                                            <table class="w-100">
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="align-middle text-ddd text-end">{{ \App\Models\Cart::whereIn('status', [1,2,3,4,5,6])->sum('price_after') }}</td>
                                                </tr>
                                                </tbody>
                                            </table>
                                        </td>
