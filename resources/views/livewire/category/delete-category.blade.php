<div class="d-inline">
    @if(auth()->check() and auth()->user()->can('services delete services'))
        <a class="btn btn-sm btn-danger rounded-circle" data-bs-toggle="modal" data-bs-target="#deleteModalcategory{{$category['id']}}" title="{{ __('Delete') }}">
            <i class="fa fa-trash"></i>
        </a>

        <div wire:ignore.self class="modal fade" id="deleteModalcategory{{$category['id']}}" tabindex="-1" role="dialog" aria-labelledby="deleteModalcategory{{$category['id']}}Label" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content bg-sub">
                    <div class="modal-header bg-sub border-secondary">
                        <h5 class="modal-title" id="deleteModal{{$category['id']}}Label">{{ __('Delete Confirm') }}</h5>
